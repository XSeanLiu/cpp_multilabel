...
bool NetNeedsDataUpgrade(const NetParameter& net_param) {
  for (int i = 0; i < net_param.layers_size(); ++i) {
    ...
    if (net_param.layers(i).type() == V1LayerParameter_LayerType_MULTI_LABEL_IMAGE_DATA) {
      MultiLabelImageDataParameter layer_param = net_param.layers(i).multi_label_image_data_param();
      if (layer_param.has_scale()) { return true; }
      if (layer_param.has_mean_file()) { return true; }
      if (layer_param.has_crop_size()) { return true; }
      if (layer_param.has_mirror()) { return true; }
...
void UpgradeNetDataTransformation(NetParameter* net_param) {
  for (int i = 0; i < net_param->layers_size(); ++i) {
    ...
    CONVERT_LAYER_TRANSFORM_PARAM(MULTI_LABEL_IMAGE_DATA, MultiLabelImageData, multi_label_image_data);
...
const char* UpgradeV1LayerType(const V1LayerParameter_LayerType type) {
  ...
  case V1LayerParameter_LayerType_IMAGE_DATA:
    return "ImageData";
  case V1LayerParameter_LayerType_MULTI_LABEL_IMAGE_DATA:
    return "MultiLabelImageData";
