...
#include "caffe/toolkit/utils.hpp"
...
void MultiLabelImageDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const int class_num = this->layer_param_.multi_label_image_data_param().class_num();
  ...
  label_shape.push_back(class_num);
...
void MultiLabelImageDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    for(size_t i = 0; i<lines_[lines_id_].second.size(); i++)
        prefetch_label[batch->label_.offset(item_id, lines_[lines_id_].second[i])] = 1;;
    // go to the next iter
    lines_id_++;
