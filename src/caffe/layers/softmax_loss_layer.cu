...
__global__ void SoftmaxLossForwardGPU(const int nthreads,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (has_ignore_label_) {
		for(size_t i = 0; i<ignore_label_size;++i)
		{
			iszero = (label_value - ignore_label[i]);
			if(!iszero)
			{
				key = true; break;
			}
...
void SoftmaxWithLossLayer<Dtype>::Forward_gpu(
  ...
  int ignore_label_size = ignore_label_.size();
  int ignore_label[ignore_label_size];
  for(size_t i = 0; i<ignore_label_size; ++i)
	ignore_label[i] = ignore_label_[i];
